...
const ONEAPI_RELEASES: Record<string, string> = {
  "2023.2":
    "https://registrationcenter-download.intel.com/akdlm/irc_nas/19163/m_fortran-compiler_p_2023.2.0.495.dmg",
  "2023.1":
    "https://registrationcenter-download.intel.com/akdlm/irc_nas/19086/m_fortran-compiler_p_2023.1.0.446.dmg",
  "2023.0":
    "https://registrationcenter-download.intel.com/akdlm/irc_nas/19005/m_fortran-compiler_p_2023.0.0.25911.dmg",
};
...
export async function installDarwin(target: Target): Promise<string> {
  ...
  for (const line of envOutput.split("\n")) {
    ...
    if (
      /^(PATH|LD_LIBRARY_PATH|DYLD_LIBRARY_PATH|.*INTEL.*|.*ONEAPI.*|.*TBB.*|.*MKL.*|.*CMPLR.*)$/i.test(
        key,
      )
    ) {
